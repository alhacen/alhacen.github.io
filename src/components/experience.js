  ...
  const experience = [
    {
      ...
      data: {
        ...
        li: "Leading Tech team<br>Leading Marketing team",
    ...
    },
    {
      ...
      data: {
        ...
        subHeader: "January 2022 - March 2023",
        li: "Shipped the Polygon Bridge for Gnosis Safe<br> Shipped New Token Mapper<br> Worked on Polygon wallet multichain architecture<br> Worked on zkEVM integration on Polygon wallet",
        // text: 'working as a full-stack and  UI UX desingner. mainly working on websocket, payment gateway and prototyping Ui components on figma',
      },
    },
    {
  ...
  return (
    <div className="flex bg-black pt-36 pb-36 justify-center overflow-hidden ">
      <div className="text-white w-full gap-5 flex flex-col mix-blend-difference">
        <div className="text-4xl text-center">The journey;</div>
        <div
          className="flex flex-col md:flex-row items-center md:items-start justify-center gap-10 flex-1"
        ...
        >
          <div className="gap-3 md:overflow-x-hidden overflow-x-scroll w-full md:w-auto md:flex-col flex-row justify-center md:justify-start flex p-8 border-r">
            {experience.map((e, index) => {
              let dir1, dir2;
              if (window.aniamteDir === undefined) window.aniamteDir = index;
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    experienceColRef.current.classList.add(dir1);
                    setSelected(index);
                    experienceColRef.current.classList.add(dir2);
                    setSelected(index);

                    setTimeout((_) => {}, 200);

                    setTimeout((_) => {
                      experienceColRef.current.classList.remove(dir1);
                      experienceColRef.current.classList.remove(dir2);
                    }, 300);
                  }}
                  className={`border-l-4 mix-blend-difference ${
                    index === selected
                      ? "border-red-400 text-red-200"
                      : "border-white text-white"
                  } cursor-pointer md:text-xl  font-bold p-2 pl-5`}
                >
                  {e.displayName}
                </div>
              );
          ...
          <div
            ref={experienceColRef}
            className={` flex flex-col w-96  items-center md:items-start `}
            style={{ animationDuration: "150ms" }}
          >
            <div className="text-xl font-bold" style={{ wordSpacing: "6px" }}>
              {experience[selected].data.header} @{" "}
              <span className="text-red-200 mix-blend-difference letterSpacing-2">
                {experience[selected].displayName}
              </span>
            </div>
